...
Page({
  ...
  onLoad: function (options) {
    var flag = options.from=='cart', that = this;
  ...
  onShow: function () {
    if(this.data.id) {
      var that = this;
      //下单后，支付成功或者失败后，点击左上角返回时能够更新订单状态 所以放在onshow中
      var id = this.data.id;
  ...
  _firstTimePay: function() {
    ...
    for(let i=0; i<productInfo.length; i++) {
      orderInfo.push({
        ...
        count: productInfo[i].couts
      });
    }
    var that = this;
    // 支付分两部步，第一步是生成订单号，然后根据订单号支付
