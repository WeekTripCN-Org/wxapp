...
Page({
  ...
  changeCategory: function(event) {
    var index = category.getDataSet(event, 'index'), id = category.getDataSet(event, 'id');
    this.setData({
  ...
  isLoadedData: function(index) {
    if(this.data['categoryInfo'+idex]) {
      return true;
    }
    return false;
  },
  ...
  getDataObjForBind: function(index, data) {
    var obj = {}, arr = [0,1,2,3,4,5], baseData=this.data.categoryTypeArr['index'];
    for(var item in arr) {
      if(item == arr[index]) {
